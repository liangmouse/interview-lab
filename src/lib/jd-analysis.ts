...
export async function parseJobDescriptionInput(input: {
  text?: string;
  file?: File | null;
}): Promise<{ success: true; text: string; sourceType: "manual" | "upload" } | { success: false; error: string }> {
...
export async function analyzeJobDescription(rawText: string): Promise<{ success: true; data: JobDescriptionAnalysis } | { success: false; error: string }> {
  try {
