...
function buildKickoffInstructions(userProfile) {
...
function shouldRetryKickoff(error) {
    return (error instanceof Error && error.message.includes("Agent activity not found"));
}
export async function sendKickoffWithRetry({ session, userProfile, maxAttempts = 5, retryDelayMs = 300, }) {
