...
export function createInterviewApplier(args) {
    ...
    const doApply = async (interview) => {
        // 1. 停止之前的监控定时器
        stopStageMonitor();
        // 如果时长缺失或异常，默认设置为 1800秒 (30分钟)
        const safeDurationMinutes = typeof interview.duration === "number" && interview.duration > 0
            ? interview.duration
            : 30;
        ...
        // 2. 加载历史面试记录 (上下文恢复)
        let historyMessages = [];
        const interviewId = typeof interview.id === "string"
            ? interview.id
            : String(interview.id || "");
        if (interviewId) {
        ...
        const visibleHistoryMessages = historyMessages.filter((msg) => {
            const role = msg === null || msg === void 0 ? void 0 : msg.role;
        ...
        if (visibleHistoryMessages.length > 0) {
            ...
            console.log(`[面试] 恢复了 ${visibleHistoryMessages.length} 条历史消息到上下文。`);
        }
        // 3. 定义 Agent 更新逻辑 (随阶段变化)
        const updateAgentForStage = (stage) => {
            const summaries = currentStageManager === null || currentStageManager === void 0 ? void 0 : currentStageManager.getStageSummaries();
            const newPrompt = buildStagePrompt(stage, userProfile, interview, summaries);
            ...
            if (currentStageManager.isStageOverTime()) {
                ...
                if (next) {
                    ...
                    (async () => {
                        ...
                        const allMessages = chatCtx.messages || chatCtx.items || [];
                        // 注意: LiveKit ChatContext.messages getter 返回只读数组
                        const stageMessages = allMessages.slice(currentStageStartMessageIndex);
                        currentStageStartMessageIndex = allMessages.length; // 重置下一阶段的索引
                        console.log(`[面试] 正在总结阶段 ${prevStage} (${stageMessages.length} 条消息)...`);
        ...
        if (visibleHistoryMessages.length === 0) {
            ...
            console.log(`[Interview] 检测到已有历史记录(${visibleHistoryMessages.length}条)，跳过开场白。`);
        }
    };
