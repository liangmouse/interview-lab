import { metrics, runWithJobContextAsync, voice, } from "@livekit/agents";
import { RoomEvent } from "@livekit/rtc-node";
import * as livekit from "@livekit/agents-plugin-livekit";
import { RoomServiceClient } from "livekit-server-sdk";
import { loadInterviewContext, loadUserContext, buildSystemPrompt, } from "../services/context-loader";
import { BASE_SYSTEM_PROMPT } from "../constants/prompts";
import { TECH_VOCABULARY, CHINESE_CONTEXT_VOCABULARY, } from "../constants/vocabulary";
...
import { createDeepgramSTT, createGeminiLLM, createGeminiTTS, } from "../config/providers";
import { saveUserMessage, saveAiMessage, } from "../services/message-persistence";
 ...
 */
async function kickOldAgents(roomName) {
    ...
    try {
        ...
        const oldAgents = participants.filter((p) => p.identity.startsWith("agent-"));
        ...
        console.log(`[kickOldAgents] Could not check room "${roomName}":`, errorMessage);
    }
}
/**
 * Agent 运行入口
 * @param ctx
 */
export async function agentEntry(ctx) {
    var _a, _b;
    // 关闭先前的会话房间
    const roomName = ((_b = (_a = ctx.job) === null || _a === void 0 ? void 0 : _a.room) === null || _b === void 0 ? void 0 : _b.name) || ctx.room.name;
    if (roomName)
        await kickOldAgents(roomName);
    // Agent连入房间
    await ctx.connect();
    // 等待第一个用户连入房间
...
export async function runAgentSession(room, participant, vad) {
    let userProfile = null;
    if (!participant.identity.startsWith("agent-")) {
        userProfile = await loadUserContext(participant.identity);
    }
    const currentPrompt = buildSystemPrompt(userProfile, BASE_SYSTEM_PROMPT, null);
    ...
    const profileKeywords = extractKeywordsFromProfile(userProfile);
    ...
    console.log(`[STT Init] Profile-specific keywords (${profileKeywords.length}):`, profileKeywords);
    // 定义Agent如何处理音频输入和输出
    ...
    class InterviewAgent extends voice.Agent {
        async onUserTurnCompleted(_chatCtx, newMessage) {
            var _a;
            if (turnMode === "manual") {
                const messageId = newMessage === null || newMessage === void 0 ? void 0 : newMessage.id;
                if (messageId) {
                    session.history.items = session.history.items.filter((item) => item.id !== messageId);
                }
                throw new voice.StopResponse();
            }
            const text = (_a = newMessage === null || newMessage === void 0 ? void 0 : newMessage.textContent) === null || _a === void 0 ? void 0 : _a.trim();
            if (text) {
    ...
    session.on(voice.AgentSessionEventTypes.UserInputTranscribed, (ev) => {
        ...
        if (turnMode === "manual")
            return;
        if ((_a = ev.transcript) === null || _a === void 0 ? void 0 : _a.trim()) {
    ...
    let resolveStartInterview = null;
    const startInterviewPromise = new Promise((resolve) => {
    ...
    const getStartInterviewPayload = (m) => {
        ...
        const nextTurnMode = modeRaw === "vad" ? "vad" : "manual";
    ...
    room.on(RoomEvent.DataReceived, async (payload) => {
        try {
            ...
            if (startPayload) {
                ...
                const interview = (await loadInterviewContext(startPayload.interviewId));
                if (interview) {
                    latestInterview = interview;
                    if (!sessionRunning)
                        pendingInterview = interview;
                    resolveStartInterview === null || resolveStartInterview === void 0 ? void 0 : resolveStartInterview(interview);
                    resolveStartInterview = null;
                    scheduleApplyLatestInterview();
                    return;
                }
                // interview 记录不存在或无权限时，立即走通用开场白兜底
                console.warn(`[RPC] Interview context not found for ${startPayload.interviewId}, fallback to generic kickoff`);
                if (!hasGreeted) {
                    hasGreeted = true;
                    startInterviewHandled = true;
